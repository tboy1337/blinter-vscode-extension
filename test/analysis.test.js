...
const { analyzeLine, buildVariableIndexFromFile, resetIssueIdCounter } = require('../lib/analysis');
...
describe('Analysis pipeline', () => {
  ...
  it('resets issue id counter for deterministic tests', () => {
    resetIssueIdCounter();
    const first = analyzeLine('Line 1: Syntax issue (E001)', {
      workspaceRoot: null,
      defaultFile: path.join(__dirname, 'fixtures', 'variable-sample.cmd'),
      variableIndex: new Map()
    }).issues[0];
    resetIssueIdCounter();
    const second = analyzeLine('Line 1: Syntax issue (E001)', {
      workspaceRoot: null,
      defaultFile: path.join(__dirname, 'fixtures', 'variable-sample.cmd'),
      variableIndex: new Map()
    }).issues[0];
