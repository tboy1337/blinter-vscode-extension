...
function clearExtensionRequireCache(repoRoot) {
  delete require.cache[path.join(repoRoot, 'extension.js')];
  const libDir = path.join(repoRoot, 'lib');
  if (fs.existsSync(libDir)) {
    for (const fileName of fs.readdirSync(libDir)) {
      if (fileName.endsWith('.js')) {
        delete require.cache[path.join(libDir, fileName)];
      }
    }
  }
}

module.exports = { createMockVscode, createRange, clearExtensionRequireCache };
