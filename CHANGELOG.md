...
- `blinter.processTimeoutMs` setting to terminate hung Blinter processes (lint, debug, and create-config)
...
- Expanded coverage gate (>=95%) across `extension.js` and all `lib/*.js` modules
...
- Debug adapter treats null/undefined process exit codes as failures (consistent with lint)
...
- Debug launch and executable resolution honor workspace-scoped `blinter.*` settings
- Cap lint and debug stdout/stderr buffers at 64 KB before newline splitting
- `blinter.run` uses the same visible-batch-editor resolution as `blinter.runAndDebug`
...
- Blinter Output webview and critical decorations use merged lint + debug issues (consistent with Problems panel)
...
- CI runs UAT checks, performance checks, and uses the latest stable Blinter vendor release unless `BLINTER_VERSION` is set
...
- Register `blinter.runAndDebug` command for `.bat` and `.cmd` files (default keybinding was `F5`; superseded by `Ctrl+F5` in v1.26.80700)
